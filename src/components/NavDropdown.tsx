...
export function NavDropdown({ logoUrl, businessName }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(v => !v)}
        className="flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-emerald-800 transition-colors"
      >
        {logoUrl ? (
          <img
            src={logoUrl.split('?')[0]}
          ...
          <div className="h-7 w-7 rounded-md bg-emerald-700 flex items-center justify-center shrink-0">
            <span className="text-white text-xs font-bold leading-none">{initials}</span>
          </div>
        )}
        <svg
          className={`w-3.5 h-3.5 text-emerald-300 transition-transform duration-150 ${open ? 'rotate-180' : ''}`}
        ...
        <div className="absolute right-0 mt-1.5 w-48 bg-white rounded-xl border border-gray-200 shadow-lg overflow-hidden z-20">
          ...
          <div className="border-t border-gray-100">
            <form action={logout}>
              <button
                type="submit"
                className="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-gray-900 hover:bg-gray-50 transition-colors"
              >
                <svg className="w-4 h-4 text-gray-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
