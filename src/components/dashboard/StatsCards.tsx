...
export function StatsCards({ stats, invoiceCount }: { stats: CurrencyStats[]; invoiceCount: number }) {
  ...
  const statCards = [
    ...
    { label: 'Amount Paid', values: stats.map(s => ({ value: s.paid, currency: s.currency })), color: 'text-[#2a5130]' },
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-white rounded-xl border border-[#e0e0e3] shadow-card p-5">
        <p className="label-caps mb-2">Total Invoices</p>
        <p className="text-xl font-bold text-gray-900 data-mono">{invoiceCount}</p>
      </div>
    </div>
  )
}
